// KOREAN CONJUGATION PRACTICE MODULE (FULLY CORRECTED)
// Fixed to properly attach consonants and contract vowels WITHIN syllables
...
const conjugationRules = {
    ...
    formalPresent: function(verb, stem) {
        if (this.hasJongseong(stem)) {
            // Stem ends with consonant: add 습니다
            return stem + '습니다';
        } else {
            // Stem ends with vowel: add ㅂ as jongseong, then 니다
            const lastChar = stem[stem.length - 1];
            const parts = this.decompose(lastChar);
    ...
    formalPast: function(verb, stem) {
        ...
        // Add ㅆ as jongseong to the last syllable, then 습니다
        const lastChar = pastStem[pastStem.length - 1];
        const parts = this.decompose(lastChar);
        const modifiedChar = this.compose(parts.choseong, parts.jungseong, 'ㅆ');
        return pastStem.slice(0, -1) + modifiedChar + '습니다';
    },
    ...
    formalFuture: function(verb, stem) {
        if (this.hasJongseong(stem)) {
            ...
            if (parts.jongseong === 'ㄹ') {
                // ㄹ drops before ㄹ: remove ㄹ jongseong, add new syllable
                const modifiedChar = this.compose(parts.choseong, parts.jungseong, '');
                return stem.slice(0, -1) + modifiedChar + ' 것입니다';
            }
            return stem + '을 것입니다';
        } else {
            // Add ㄹ as jongseong to last syllable
            const lastChar = stem[stem.length - 1];
            const parts = this.decompose(lastChar);
            const modifiedChar = this.compose(parts.choseong, parts.jungseong, 'ㄹ');
            return stem.slice(0, -1) + modifiedChar + ' 것입니다';
        }
    },
    ...
    politeFuture: function(verb, stem) {
        if (this.hasJongseong(stem)) {
            ...
            if (parts.jongseong === 'ㄹ') {
                // ㄹ drops before ㄹ: remove ㄹ jongseong
                const modifiedChar = this.compose(parts.choseong, parts.jungseong, '');
                return stem.slice(0, -1) + modifiedChar + ' 거예요';
            }
            return stem + '을 거예요';
        } else {
            // Add ㄹ as jongseong to last syllable
    ...
    getPoliteForm: function(verb, stem) {
        ...
        if (stem.endsWith('하')) {
            // Contract 하 + 여 → 해
            const parts = this.decompose('하');
            const contracted = this.compose(parts.choseong, 'ㅐ', ''); // 해
            return stem.slice(0, -1) + contracted + '요';
        }
        ...
        // Determine 아 vs 어 based on vowel harmony
        const suffix = this.isBrightVowel(lastVowel) ? '아' : '어';
        
        // Handle vowel contractions WITHIN THE SAME SYLLABLE
        ...
        // No contraction: add suffix as new syllable
        // Create new syllable with ㅇ initial
        const suffixSyllable = this.compose('ㅇ', suffix === '아' ? 'ㅏ' : 'ㅓ', '');
        return conjugatedStem + suffixSyllable + '요';
    },
    ...
    getPastStem: function(verb, stem) {
        ...
        if (stem.endsWith('하')) {
            // Contract 하 + 였 → 했
            const parts = this.decompose('하');
            const contracted = this.compose(parts.choseong, 'ㅐ', 'ㅆ'); // 했
            return stem.slice(0, -1) + contracted + this.compose('ㅇ', 'ㅓ', '');
        }
        ...
        if (!parts.jongseong) { // Only if no final consonant
            ...
            if (contracted) {
                // Replace the vowel and add ㅆ as jongseong
                const newChar = this.compose(parts.choseong, contracted, 'ㅆ');
                return conjugatedStem.slice(0, -1) + newChar + this.compose('ㅇ', 'ㅓ', '');
            }
        }
        
        // No contraction: add past marker as new syllable with ㅆ jongseong
        const pastSyllable = this.compose('ㅇ', pastMarker === '아' ? 'ㅏ' : 'ㅓ', 'ㅆ');
        return conjugatedStem + pastSyllable + this.compose('ㅇ', 'ㅓ', '');
    },
    ...
    handleIrregulars: function(verb, stem, brightSuffix, darkSuffix) {
        ...
        if (this.IRREGULAR_ㅂ.includes(verb)) {
            ...
            if (parts.jongseong === 'ㅂ') {
                // Remove ㅂ and add 우 as new syllable
                const newLastChar = this.compose(parts.choseong, parts.jungseong, '');
                const wooSyllable = this.compose('ㅇ', 'ㅜ', '');
                return stem.slice(0, -1) + newLastChar + wooSyllable;
            }
        }
        ...
        if (this.IRREGULAR_ㄷ.includes(verb)) {
            ...
            if (parts.jongseong === 'ㄷ') {
                // Replace ㄷ with ㄹ in jongseong position
        ...
        if (this.IRREGULAR_ㅡ.includes(verb)) {
            ...
            if (parts.jungseong === 'ㅡ' && !parts.jongseong) {
                ...
                if (stem.length > 1) {
                    ...
                    if (prevParts) {
                        // Remove the ㅡ syllable entirely
                        return stem.slice(0, -1);
                    }
                }
                // If only one syllable, still drop ㅡ
                return stem.slice(0, -1);
            }
        }
    ...
    contractVowels: function(vowel1, suffix) {
        // Contractions that create complex vowels or same vowels
        const contractions = {
            // ㅏ + 아 = ㅏ (no change)
            'ㅏ아': 'ㅏ',
            // ㅓ + 어 = ㅓ (no change)
            ...
            // ㅏ + 어 = ㅏ (no change)
            'ㅏ어': 'ㅏ',
            // 하 + 여 = 해 (special case - handled separately)
            'ㅏ여': 'ㅐ'
        };
...
function checkConjugation(spokenText) {
    ...
    // Normalize spacing (remove all spaces for comparison)
    const spokenNormalized = spokenText.replace(/\s+/g, '');
    const correctNormalized = correctAnswer.replace(/\s+/g, '');
    
    // Use pronunciation engine to compare
    const similarity = pronunciationEngine.getPronunciationSimilarity(spokenNormalized, correctNormalized);
    const isCorrect = similarity >= 0.85; // 85% similarity threshold
    
    // Also check if pronunciation matches exactly
    const pronunciationMatch = pronunciationEngine.pronounciationEquals(spokenNormalized, correctNormalized);
    
    const feedbackArea = document.getElementById('feedback-area');
    
    // Get pronunciations for display
    const spokenPronunciation = pronunciationEngine.getPronunciation(spokenNormalized);
    const correctPronunciation = pronunciationEngine.getPronunciation(correctNormalized);
